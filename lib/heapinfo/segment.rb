module HeapInfo
  class Segment
    ...
    # @param [Regexp, String] pattern The segment name want to match in maps. If <tt>String</tt> is given, the pattern need to be exactly the segment's name. 
    ...
    def self.find(maps, pattern)
      needs = maps.select{|m| pattern.is_a?(Regexp) ? m[3] =~ pattern : m[3] == pattern}
